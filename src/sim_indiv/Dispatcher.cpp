...
void Dispatcher::open_trans_record() {
  ...
  if (!transmission_record.is_open()) {
    cpp11::stop("unable to create transmission record at specified location. Check the path exists, and that you have write access");
  }
...
void Dispatcher::run_simulation(cpp11::list &args_progress) {
  ...
  double z = 100.0;
  for (int i = 0; i < params->max_time; ++i) {
    update_progress(args_progress, "pb_sim", i, params->max_time, true);
    for (int j = 0; j < 1e8; ++j) {
      z = pow(z, 0.9);
    }
  }
