...
To make this clearer, here is an example. Imagine we want to model a population that has good access to healthcare overall, but with wide disparity between individuals. We choose parameter values `treatment_seeking_mean = 0.7` and `treatment_seeking_sd = 0.2`, which we can visualise using the `plot_treatment_seeking()` function:
