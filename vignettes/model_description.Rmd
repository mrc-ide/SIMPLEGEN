...
The first step in the SIMPLEGEN pipeline is to simulate infection events from an epidemiological transmission model. Any transmission model can be used, just as long as it produces output in a particular format, but a built-in transmission model is included with the program to make life easier. This built-in model is designed to be as simple as possible but no simpler, and does not attempt to be a complete phenomenological description of the malaria life cycle.

This vignette gives a high-level description of the built-in transmission model.
