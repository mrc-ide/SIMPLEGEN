...
We will compare this against model output obtained by taking a range of values of mosquito population size relative to human population size. Measured values include the annualised EIR over the whole population, and the prevalence by microscopy (acute + chronic) in children less than 15 years of age. For each simulation, model output is run for 20 years and values are taken as the average over the final 10 years.

```{r}
# load precomputed model output
# code to produce this output: R_ignore/Calibration/model_calibration_prev_inc1.R
df_model <- readRDS("../inst/extdata/model_calibration_prev_inc1.rds")
```
