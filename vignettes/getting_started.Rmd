...
Alternatively, if you are familiar with the pipe operator `%>%` from the `magrittr` package then you can use this to chain together multiple commands:
...
```
As before, we load this into the project using the `define_epi_sampling_parameters()` function:
